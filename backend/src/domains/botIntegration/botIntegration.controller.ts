...
import {
  ackBotEncounterEvents,
  ackBotMarketEvents,
  getBotEncounterEvents,
  getBotMarketEvents,
} from './botIntegration.service';
...
export async function getBotEncounterEventsHandler(req: Request, res: Response) {
  try {
    const data = await getBotEncounterEvents({
      afterIdRaw: req.query?.after_id,
      limitRaw: req.query?.limit,
      integrationKey: req.header('x-integration-key'),
    });
...
export async function ackBotEncounterEventsHandler(req: Request, res: Response) {
  try {
    const data = await ackBotEncounterEvents({
      eventIds: req.body?.event_ids,
      consumerRaw: req.body?.consumer,
      integrationKey: req.header('x-integration-key'),
    });
