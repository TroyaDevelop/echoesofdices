...
export async function ensureRuntimeSchema(): Promise<void> {
  ...
  await query(
    "CREATE TABLE IF NOT EXISTS friendships (id INT PRIMARY KEY AUTO_INCREMENT, user_id_1 INT NOT NULL, user_id_2 INT NOT NULL, status ENUM('pending', 'accepted') DEFAULT 'pending', created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP, updated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP, FOREIGN KEY (user_id_1) REFERENCES users(id) ON DELETE CASCADE, FOREIGN KEY (user_id_2) REFERENCES users(id) ON DELETE CASCADE, UNIQUE KEY unique_friendship (user_id_1, user_id_2))",
    []
  );

  await query(
    "CREATE TABLE IF NOT EXISTS notifications (id INT PRIMARY KEY AUTO_INCREMENT, user_id INT NOT NULL, type VARCHAR(50) NOT NULL, data JSON, is_read BOOLEAN DEFAULT FALSE, created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP, FOREIGN KEY (user_id) REFERENCES users(id) ON DELETE CASCADE)",
    []
  );
  ...
  await safeQuery('ALTER TABLE user_character_sheets ADD COLUMN IF NOT EXISTS spell_slots_json TEXT', []);
  ...
  await query(
    'CREATE TABLE IF NOT EXISTS spell_schools (id INT PRIMARY KEY AUTO_INCREMENT, name VARCHAR(80) NOT NULL UNIQUE, created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP, updated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP)',
    []
  );

  await safeQuery("INSERT IGNORE INTO spell_schools (name) VALUES ('Воплощение'), ('Вызов'), ('Иллюзия'), ('Некромантия'), ('Ограждение'), ('Очарование'), ('Преобразование'), ('Прорицание')", []);
  ...
  await query('ALTER TABLE spells ADD COLUMN IF NOT EXISTS school_extra VARCHAR(120)', []);
  ...
  await safeQuery('ALTER TABLE screen_encounters ADD INDEX idx_screen_encounters_started_by (started_by)', []);
