...
export default function MarketPage() {
  ...
  const filteredItems = useMemo(() => {
    ...
    return items.filter((it) => {
      ...
      const cat = normalizeQuery(categoryLabel(it?.category));
      ...
      <div className="space-y-6">
        <div className="flex items-end justify-between gap-4 flex-wrap">
          <div>
            ...
            <p className="mt-2 text-slate-300">Выберите регион — цена будет рассчитана автоматически.</p>
          </div>
          ...
          <div className="w-full sm:w-[34rem] flex flex-col sm:flex-row gap-3">
            ...
            <input
              ...
              placeholder="Поиск (предмет или категория)"
              className="w-full rounded-xl border border-white/10 bg-white/5 px-4 py-2 text-slate-100 placeholder:text-slate-400 focus:outline-none focus:ring-2 focus:ring-purple-500"
            />
          </div>
        </div>
