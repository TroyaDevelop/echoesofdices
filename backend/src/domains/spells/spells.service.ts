...
export async function createSpellRecord(body: any) {
  ...
  const result = await insertSpell({
    ...
    school_extra: school_extra ? String(school_extra).trim() : null,
    theme: themeValue,
...
export async function updateSpellRecord(id: number, body: any) {
  ...
  const merged = {
    ...
    school_extra: normOpt(body.school_extra) === undefined ? existing.school_extra : normOpt(body.school_extra),
